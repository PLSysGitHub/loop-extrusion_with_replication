...
cdef inline np.int64_t sample_loading_point(np.float64_t [:] weights, np.int64_t [:] occupied):
    ...
    for i in occupied:
        if i >= 0:
            ps[i] = 0
            #because binding legs at (i, i+1), also need to set binding at i-1 to zero.
            if i<L: #chrom copy 0
                ps[(i-1+L)%L] = 0 
            else:
                ps[(i-1+L)%L+L] = 0
...
cdef class System:
    ...
    def __cinit__(self, L_replicated, L, N, vels, rebinding_times, unbinding_rates,
                  fork_rate=0, init_locs=None, perms=None, binding_rates=None):
        self.L = L
        self.L_replicated = L_replicated
        self.N = N
        self.N_max = N*2*L//(L+L_replicated)
        self.fork_rate=fork_rate

        self.fork=np.array([L-L_replicated//2-1, L_replicated//2], dtype=np.int64) #similar to LEs, first index moves left, second index right

        self.lattice = -1*np.ones((2,L), dtype=np.int64) #-1 means unoccupied. First index is chromosome, second lattice site.
    ...
    cdef np.int64_t move_leg(System self, np.int64_t leg_idx, np.int64_t new_pos):
        cdef np.int64_t chrom_copy
        cdef np.int64_t prev_pos, prev_chrom_copy 

        chrom_copy = int(new_pos>=self.L) #1 if on replicated chromosome, 0 otherwise

        if (new_pos >= 0) and (self.lattice[chrom_copy, new_pos%self.L ] >=0): #if moving to an occupied site
            print("move leg ", leg_idx, " to occupied site ", new_pos, ":",self.lattice[chrom_copy, new_pos%self.L])
            return 0
        
        if self.fork_rate>0 and (new_pos==self.fork[1] or new_pos==self.fork[0]): #moving to a fork during replication
            print("move to fork")
            return 0
        ...
        if prev_pos >= 0:
            if self.lattice[prev_chrom_copy, prev_pos%self.L] < 0:
                print(leg_idx, " wasnt attached at prev site ", prev_pos)
                return 0
            self.lattice[prev_chrom_copy, prev_pos%self.L] = -1

        if new_pos >= 0:
            self.lattice[chrom_copy, new_pos%self.L] = leg_idx
...
cdef regenerate_event(System system, Event_heap evheap, np.int64_t event_idx):
    ...
    cdef np.int64_t direction, chrom_copy
    cdef np.int64_t pos1, pos2
    ...
    if (event_idx < 4 * system.N_max) :
        ...
        if loop_idx<system.N and (system.locs[leg_idx] >= 0):            
            ...
            if new_position%system.L==system.fork[0] or new_position%system.L==system.fork[1]:
                local_vel=0
            else:
                local_vel = (system.perms[system.locs[leg_idx]%system.L + (direction+1)//2]
                    * system.vels[leg_idx + (direction+1)*system.N_max])

            if local_vel > 0:
                if system.lattice[chrom_copy, new_position%system.L]<0: #next site unoccupied
                    evheap.add_event(
                        event_idx,
    ...
    # Passive unbinding.
        ...
        if loop_idx<system.N and (pos1 >= 0) and (pos2 >= 0): #if attached
            if system.fork_rate>0 and (pos1==system.fork[0] or pos1==system.fork[1] or pos2==system.fork[0] or pos2==system.fork[1]):
    ...
    # Rebinding from the solution to a random site.
        ...
        if system.fork_rate>0 and system.fork[0]!=system.fork[1]: #ongoing but incomplete
...
cdef np.int64_t regenerate_after_fork_move(System system, Event_heap evheap, np.int64_t fork_idx):
    cdef np.int64_t leg_idx, loop_idx, fork_pos, move_direction
    cdef np.int64_t status = 1
    ...
    if system.lattice[1, fork_pos]>=0:
        print("There were LEFs on unreplicated site!")
        return 0

    # Regenerate movement rate in front of the fork
    if system.lattice[0, fork_pos+move_direction]>=0:
        leg_idx=system.lattice[0, fork_pos+move_direction]
        regenerate_event(system, evheap, leg_idx+2*system.N_max*(1-fork_idx)) #if fork moves left, regenerate right. If fork moves right, regenerate left.

    #Regenerate movement rate two steps behind fork
    if fork_pos-2*move_direction>=0 and fork_pos-2*move_direction<system.L: #two steps behind is a part of lattice
        if system.lattice[0, fork_pos-2*move_direction]>=0:
            leg_idx=system.lattice[0, fork_pos-2*move_direction]
        ...
        if system.lattice[1, fork_pos-2*move_direction]>=0:
            leg_idx=system.lattice[1, fork_pos-2*move_direction]
            regenerate_event(system, evheap, leg_idx+2*system.N_max*fork_idx) #if fork moves left, regenerate left. If fork moves right, regenerate right.
...
cdef np.int64_t do_event(System system, Event_heap evheap, np.int64_t event_idx) except 0:
    ...
    cdef np.int64_t new_pos, leg_idx, prev_pos, prev_pos2, direction, loop_idx, chrom_copy
    ...
    if event_idx < 4 * system.N_max:
        ...
        status = 1
        if (prev_pos >= 0) and new_pos%system.L!=system.fork[0] and new_pos%system.L!=system.fork[1]:
            if (new_pos>=system.L) != (prev_pos>=system.L): #if moving between replicated and unreplicated
                print("Trying to move between replicated and unreplicated regions! Old pos: ", prev_pos, " new pos: ", new_pos)
                return 0

            # make a step only if there is no boundary and if the next site is unoccupied.
            if (system.perms[prev_pos%system.L + (direction + 1) // 2] > 0) and (system.lattice[chrom_copy, new_pos%system.L] < 0):
        ...
        if system.fork_rate>0:
            #replication ongoing. don't load at forks or unreplicated regions
            new_pos = sample_loading_point(system.binding_affinities, np.concatenate((system.locs,
                                        system.fork, np.arange(system.fork[1]+system.L, system.fork[0]+system.L+1))))
        else:
            #replication not on-going. but don't load on unreplicated regionsi
            new_pos = sample_loading_point(system.binding_affinities, np.concatenate((system.locs, 
                                        np.arange(system.fork[1]+system.L, system.fork[0]+system.L+1))))
    
        chrom_copy = int(new_pos>=system.L)
        new_pos_2=(new_pos+1)%system.L+chrom_copy*system.L

        if system.lattice[chrom_copy, new_pos%system.L]>=0:
            print("binding leg 1 to occupied site: ", new_pos)
        ...
        if system.lattice[chrom_copy, new_pos_2%system.L]>=0:
            print("binding leg 2 to occupied site: ", new_pos_2)
            print("leg_id=", system.lattice[chrom_copy, new_pos_2%system.L], " with locs entry ", system.locs[system.lattice[chrom_copy, new_pos_2%system.L]])
            status=0

        # rebind the loop
        status *= system.move_leg(loop_idx, new_pos)
        status *= system.move_leg(loop_idx+system.N_max, new_pos_2)
        ...
        status = 4
        if system.fork_rate>0 and system.fork[0]!=system.fork[1]:
...
cpdef simulate(p, verbose=True):
    ...
    cdef np.int64_t L_REPLICATED = p.get('L_REPLICATED', 0)
    ...
    cdef np.int64_t N_MAX = N*2*L//(L+L_REPLICATED)
    ...
    cdef System system = System(L_REPLICATED, L, N, VELS, REBINDING_TIMES, UNBINDING_RATES, 0, INIT_LOCS, PERMS, BINDING_RATES)
