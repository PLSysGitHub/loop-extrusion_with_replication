...
cdef class System:
    ...
    cdef np.int64_t next_pos(System self, np.int64_t leg_idx, np.int64_t direction):
        ...
        if self.L_replicated>0 and prev_pos>=self.L and new_pos not in self.fork: #on the replicated chromosome, not fork
            new_pos+=self.L
    ...
    cdef np.int64_t move_leg(System self, np.int64_t leg_idx, np.int64_t new_pos):
        ...
        if chrom_copy==1 and (new_pos<=self.fork[0]+self.L and new_pos>=self.fork[1]+self.L):
        ...
        if prev_pos >= 0:
            if self.lattice[prev_chrom_copy, prev_pos%self.L+lattice_increment] < 0:
...
cdef regenerate_neighbours(System system, Event_heap evheap, np.int64_t pos):
    ...
    #if we move to a fork, we should regenerate moves for SMCs on other chromosome copy
    if -1<pos<system.L and pos in system.fork:
        regenerate_neighbours(system, evheap, pos+system.L)
...
cdef np.int64_t do_event(System system, Event_heap evheap, np.int64_t event_idx) except 0:
    ...
    if event_idx < 4 * system.N_max:
        ...
        status = 1
        if (prev_pos >= 0):
            if chrom_copy==1 and (new_pos<=system.fork[0]+system.L and new_pos>=system.fork[1]+system.L):
                print("Trying to move to unreplicated site! Old pos: ", prev_pos, " new pos: ", new_pos, ". Fork: ", system.fork[0], system.fork[1])
                return 0
        ...
        if chrom_copy==1 and (new_pos<=system.fork[0]+system.L and new_pos>=system.fork[1]+system.L):
            print("binding to unreplicated site: ", new_pos)
            print("with fork at: ", system.fork[0], " ", system.fork[1])
            status=0
