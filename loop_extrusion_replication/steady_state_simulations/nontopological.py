...
from ..translocators.nontopological import smcForkTranslocator
...
def initModel(R,knockOffProb,kOriToTer_kTerToOri_kBypass,parSsites,\
    ...
    stallFork : float
    The probability that the SMC complex will stall at the fork
    ...
    SMCTran = smcForkTranslocator(R//2, birthArray, deathArray, stallLeftArray, \
            stallRightArray, pauseArrayL, pauseArrayR, \
            stallDeathArray, kOriToTer_kTerToOri_kBypass,  smcNum,0,stallFork)
    return SMCTran
...
def run_simulation(R,monomer_size,monomer_wig, knockOffProb, kOriToTer_kTerToOri_kBypass, parSsites, wind, terSites,\
        ...
        col_rate=0.1, trunc=0.5, stallFork=0., num_tethers=2,top_monomer=0,start_segregated=False ):
    """
    ...
    monomer_size : float
    The size of the monomer in nm

    monomer_wig : float
    The size of the monomer in simulation units
    ...
    trunc : float
    Sets the excluded volume strength for the polymer simulations

    stallFork : float
    The probability of stalling a LE at the fork

    num_tethers : int
    The number of tethers to be used in the simulation, between 0 and 2

    top_monomer : int
    The monomer number at which to tether the chromosome

    start_segregated : bool
    Whether to start the chromosome in a segregated state
