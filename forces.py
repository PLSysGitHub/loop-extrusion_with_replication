...
def polynomial_repulsive_with_exclusions(
    sim_object, trunc, radiusMult=1.0, name="polynomial_repulsive_with_exclusions"
):             
    """This is a simple polynomial repulsive potential. It has the value
    of `trunc` at zero, stays flat until 0.6-0.7 and then drops to zero
    together with its first derivative at r=1.0.
    ...
    nbCutOffDist = radius
    repul_energy = (     
        "rsc12 * (rsc2 - 1.0) * REPe1*REPe2 / emin12 + REPe1*REPe2;"
        "rsc12 = rsc4 * rsc4 * rsc4;"
        "rsc4 = rsc2 * rsc2;"
        "rsc2 = rsc * rsc;"
        "rsc = r / REPsigma * rmin12;"
    ...
    force.addPerParticleParameter("REPe")
    force.addGlobalParameter("REPsigma", radius)
    # Coefficients for x^8*(x*x-1)
    # force.addGlobalParameter('emin12', 256.0 / 3125.0)
    # force.addGlobalParameter('rmin12', 2.0 / np.sqrt(5.0))
    ...
    for i in range(sim_object.N):
        force.addParticle([trunc[i]*sim_object.kT])
               
    force.setCutoffDistance(nbCutOffDist)
               
    return force
def polynomial_repulsive_vary_trunc_r(
    sim_object, trunc, radiusMult=1.05, maxRadius=2, name="polynomial_repulsive_vary_trunc_r"
):                        
    """This is a simple polynomial repulsive potential. It has the value
    of `trunc` at zero, stays flat until 0.6-0.7 and then drops to zero
    together with its first derivative at r=1.0.
    ...
    nbCutOffDist = radius 
    repul_energy = (      
        "step(max(REPsigma1, REPsigma2)-r) * (rsc12 * (rsc2 - 1.0) * REPe1*REPe2 / emin12 + REPe1*REPe2);"
        "rsc12 = rsc4 * rsc4 * rsc4;"
        "rsc4 = rsc2 * rsc2;"
        "rsc2 = rsc * rsc;"
        "rsc = r / max(REPsigma1,REPsigma2) * rmin12;"
    )                     
    ...
    force.addPerParticleParameter("REPe")
    force.addPerParticleParameter("REPsigma")
    force.addGlobalParameter("emin12", 46656.0 / 823543.0)
    force.addGlobalParameter("rmin12", np.sqrt(6.0 / 7.0))
                          
    for i in range(sim_object.N):
        force.addParticle([trunc[i]*sim_object.kT, radius])
                          
    force.setCutoffDistance(maxRadius*sim_object.conlen*radiusMult)
...
def rounded_cap_cylindrical_confinement(sim_object, r, bottom=None, k=0.1, top=9999, name="rounded_cap_cylindrical_confinement"):
    """As it says."""
    ...
    if bottom is not None:
        force = openmm.CustomExternalForce(
            "kt * k * ("
            " step(dr) * (sqrt(dr*dr + t*t) - t)"
            " + step((bottom+r)-z) * step(dbottom) * (sqrt(dbottom^2 + t^2) - t) "
            " + step(z-(top-r)) * step(dtop) * (sqrt(dtop^2 + t^2) - t)"
            ") ;"
            "dr = sqrt(x^2 + y^2 + tt^2) - r + 10*t;"
            "dbottom=sqrt(x^2 + y^2 + (z -(bottom + r))^2) - r + 10*t;"
            "dtop=sqrt(x^2 + y^2 + (z - (top - r))^2) - r + 10*t"
        )
    else:
        force = openmm.CustomExternalForce(
            "kt * k * step(dr) * (sqrt(dr*dr + t*t) - t);" "dr = sqrt(x^2 + y^2 + tt^2) - r + 10*t"
