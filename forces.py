    ...
    for bond_idx, (i, j) in enumerate(bonds):
        if (i >= sim_object.N) or (j >= sim_object.N):
            raise ValueError(
                ...
                "are beyond the polymer length %d" % (i, j, sim_object.N)
            )
        ind= force.addBond(
            int(i), int(j), float(bondLength[bond_idx]), float(kbond[bond_idx])
        )
        #print("added force ", int(i), ",", int(j),". It has index",ind)
    return force
...
def cylindrical_confinement_part(
    sim_object, r, monomers, bottom=None, k=0.1, top=9999, name="cylindrical_confinement_part"
):
    """As it says."""
