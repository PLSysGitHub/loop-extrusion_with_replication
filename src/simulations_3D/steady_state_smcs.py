...
def load_smcs_from_h5(filename, t):
...
class simulationBondUpdater(object):
    ...
    def __init__(self,N,smcTrajectoryFile, trunc, dt=1/60):
        """
    ...
    def LEF_simulation(self, bondForce, smcTimeSteps = 100):
        ...
        for timeStep in range(smcTimeSteps):
            t=timeStep*self.dt
            left, right=load_smcs_from_h5(self.smcFile, t) 
...
def run_simulation(smcTrajFolder, bacterium, monomer_wig,\
    ...
    Args:
        ...
        no_confinement (bool) : whether to add confinement
        infinite_tube (bool) : whether to add infinite tube confinement
        dt_1D (float) : time step for 1D simulations in minutes
    Returns:
        None
    """
    ...
    for BondUpdaterCount in range(numSims):
        ...
        BondUpdater = simulationBondUpdater(N, smcFile, trunc, dt_1D)
