...
import pyximport; pyximport.install(
    ...
    reload_support=True)
from looplib_bacterial import replicating_bacterial_bypassing
...
def out_dir_name_1D(bacterium, M, GPU, results_dir="Results_1D"):
    return os.path.join(results_dir, f"Replicating_{bacterium.name}_N_{bacterium.N}_M_{M}_loopsize_{bacterium.loopSize:.4g}_ter_size_{bacterium.terLength}_ter_strength_{bacterium.terStrength}_GPU_{GPU}")
...
def run_replicating_1D_sims(bacterium, num_smcs, burn_in_time_mins, sim_time_mins, num_sims, delta_t_sec=2, GPU=0, skip_gens=0):
    ...
    for i in range(num_sims+skip_gens):
        ...
        l_sites, r_sites, forks, ts = replicating_bacterial_bypassing.simulate(p, verbose=False) #perform a new simulation
