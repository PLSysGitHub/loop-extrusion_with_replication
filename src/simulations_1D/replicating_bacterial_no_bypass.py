...
def out_dir_name_1D(bacterium, M, GPU, results_dir="Results_1D"):
    return os.path.join(results_dir, f"Replicating_no_bypass_{bacterium.name}_N_{bacterium.N}_M_{M}_loopsize_{bacterium.loopSize}_ter_size_{bacterium.terLength}_ter_strength_{bacterium.terStrength}_GPU_{GPU}")
